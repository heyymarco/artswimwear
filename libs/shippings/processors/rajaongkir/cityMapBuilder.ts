...
const results = (
    ...
    .reduce((states, city) => {
        try {
            let cities = states.get(city.province);
            if (cities === undefined) {
                cities = [];
                states.set(city.province, cities);
            } // if
            cities.push(city);
        }
        finally {
...
const cityPaths = (
    ...
    .flatMap(([state, cities]) =>
        cities.map((city): [string, string] =>
            [`${state.trim().toLowerCase()}/${city.city_name.trim().toLowerCase()}`, city.city_id]
        )
    )
);
cityPaths.sort();
...
// }
const cityMap = new Map<string, string>(cityPaths);
console.log(cityMap);